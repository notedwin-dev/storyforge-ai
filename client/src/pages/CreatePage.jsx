...
export default function CreatePage() {
  ...
  const [savedStoryData, setSavedStoryData] = useState({
    id: null,
    visibility: "private",
  });
  ...
  const transformJobToStory = (job) => {
    ...
    if (jobCharacter) {
      characters = [
        {
          name: jobCharacter.name || story.character?.name || "Character",
          description:
            jobCharacter.description || story.character?.description || "",
          traits: jobCharacter.traits || story.character?.traits || [],
          imageUrl:
            jobCharacter.imageUrl ||
            jobCharacter.avatar_url ||
            story.character?.imageUrl ||
            null,
        },
      ];
    }
    // Check if we have character data from the story itself
      ...
      characters = [
        {
          ...
          imageUrl:
            story.character.imageUrl || story.character.thumbnailUrl || null,
        },
      ];
    }
    ...
    // Handle storyboard URLs - new structure has objects with imageUrl property
    const storyboardUrls = result.storyboard_urls
      ? result.storyboard_urls.map((item) => {
          if (typeof item === "string") {
            return item; // Legacy string format
          }
          return item.imageUrl || item; // New object format
        })
      : [];

    // Handle scene URLs (if different from storyboard)
    const sceneUrls = result.scene_urls || storyboardUrls;

    return {
      id: job.id,
      title: story.title || "Your Generated Story",
      content: fullContent, // This is for backward compatibility
      fullStory: story.fullStory || fullContent, // Add the fullStory field directly
      scenes: story.scenes || [],
      ...
      length: "short",
      createdAt: createdAt,
      estimatedDuration: story.scenes
        ? `${story.scenes.length * 2}-${story.scenes.length * 3} min`
        : "5-10 min",
      videoUrl: result.video_url,
      storyboardUrls: storyboardUrls,
      sceneUrls: sceneUrls,
      audio_narration: result.audio_narration || null,
  ...
  const handleBack = () => {
    ...
    if (currentStep > 0) {
      ...
      if (newStep === 0) {
        ...
        console.log(
          "Reset hasAutoAdvanced to false and cleared character from store only"
        ); // Debug log
  ...
  const handleStorySubmit = async (promptData) => {
    ...
    try {
      ...
      const response = await storyAPI.generate({
        ...
        dna_id:
          updatedStoryData.character.dna_id || updatedStoryData.character.id,
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          title: storyData.character?.name
            ? `${storyData.character.name}'s Story`
            : "My AI Story",
          url: shareUrl,
        });
      } else {
  ...
  const handleComplete = async () => {
    try {
      ...
      if (storySaved) {
        toast.success(
          "🎉 Story creation completed! Your story has been saved successfully."
        );
      ...
      setTimeout(
        ...
        storySaved ? 2500 : 1500
      ); // Longer delay if we're opening a new tab
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Transform your character into an amazing animated story in just a
            few simple steps.
          </p>
        </div>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            {STEPS.map((step, index) => {
              ...
              const isAccessible =
                index <= currentStep && (index !== 3 || canAccessPreview());
                ...
                <div
                  ...
                  <div className="flex items-center w-full">
                    <motion.button
                      ...
                      title={
                        index === 3 && !canAccessPreview()
                          ? "Complete story generation to access preview"
                          : ""
                      }>
                      {isCompleted ? (
          ...
          <AnimatePresence mode="wait">
            <motion.div
                ...
                <div className="space-y-6">
                  {generationInProgress && generationJobId ? (
                    ...
                    <StoryPreview
                      story={
                        activeJob
                          ? transformJobToStory(getJob(activeJob))
                          : null
                      }
              ...
              {currentStep === 3 && (
                <StorySaveOptions
                  story={
                    activeJob ? transformJobToStory(getJob(activeJob)) : null
                  }
